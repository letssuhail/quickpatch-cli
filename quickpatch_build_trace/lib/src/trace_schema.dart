...
/// consumer (quickpatch_cli's `build_trace_summary.dart`).
///
/// Once a name lands in a shipped flutter or aot_tools, quickpatch_cli
/// has to understand it across every version pin that ships with a
/// QuickPatch release. So:
///
/// * **Never rename** a constant here. Add a new one and have quickpatch
///   recognize both.
/// * **Don't reuse** a removed constant's value for a different meaning
///   for the same reason.
/// * The Groovy init script (`quickpatch_trace_init.gradle` in the
...
///
/// Producers emit via [wireName]. Consumers (quickpatch_cli) parse via
...
enum TraceCategory {
  ...
  /// Per-task events emitted by `quickpatch_trace_init.gradle`.
...
/// `quickpatch_trace_init.gradle`, emitted in each gradle_task event's
...
class TraceNames {
  ...
  /// quickpatch matches with `startsWith`.
  ...
  /// Full span name quickpatch matches is `"pod install: <phase>"`.
...
/// (quickpatch_cli) matches the assembled span name against this enum's
/// [wireName]s.
