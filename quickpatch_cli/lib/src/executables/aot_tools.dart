...
Verify that `quickpatch patch` was invoked with the exact same --dart-define
(and --dart-define-from-file) flags as `quickpatch release`, and that the
...
/// Wrapper around the quickpatch `aot-tools` executable.
class AotTools {
  ...
  static bool usesLinker(String flutterRevision) {
    ...
    // This will be wrong if we ever need new quickpatch to work with old local
    // engine build.
  ...
  Future<QuickPatchProcessResult> _exec(
    List<String> command, {
    ...
    // Trace Event Format file as Flutter and quickpatch_cli. Prepended
    ...
    // Similar to [QuickPatchProcess.run] but uses [QuickPatchProcess.start]
    // instead and captures the live stdout and stderr of the process.
    Future<QuickPatchProcessResult> execute(
      String exe,
      List<String> args, {
      String? workingDirectory,
    }) async {
    ...
    // Fallback behavior for older versions of quickpatch where aot-tools was
    ...
    if (extension != '.dill' && extension != '.dart') {
      ...
      result = await execute(quickpatchEnv.dartBinaryFile.path, [
        'run',
        artifactPath,
        ...tracedCommand,
      ], workingDirectory: workingDirectory);
