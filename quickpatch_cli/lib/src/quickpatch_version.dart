...
/// A reference to a [QuickPatchVersion] instance.
final quickpatchVersionRef = create(QuickPatchVersion.new);

/// The [QuickPatchVersion] instance available in the current zone.
QuickPatchVersion get quickpatchVersion => read(quickpatchVersionRef);

/// {@template quickpatch_version}
/// Provides information about installed and available versions of QuickPatch.
/// {@endtemplate}
class QuickPatchVersion {
  String get _workingDirectory => p.dirname(Platform.script.toFilePath());

  /// Whether the current version of QuickPatch is the latest available.
  ...
  /// Returns the remote HEAD quickpatch hash.
  ///
  ...
  /// Returns the local HEAD quickpatch hash.
  ///
  /// Exits if HEAD isn't pointing to a branch, or there is no upstream.
  ...
  /// Whether the current version of quickpatch is tracking the stable branch. If
  /// we are not tracking stable, we should not check for newer versions of
  /// quickpatch or try to auto-update.
