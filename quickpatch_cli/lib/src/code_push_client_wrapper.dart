...
class CodePushClientWrapper {
  ...
  Future<App> createApp({required int organizationId, String? appName}) async {
    ...
    if (appName == null) {
      final defaultAppName = quickpatchEnv.getPubspecYaml()?.name;
...
This app may not exist or you may not have permission to view it.''');
  ...
  void ensureReleaseIsNotActive({
    ...
    if (release.platformStatuses[platform] == ReleaseStatus.active) {
      final uri = QuickPatchWebConsole.appReleaseUri(release.appId, release.id);
      logger.err(
        '''
It looks like you have an existing ${platform.name} release for version ${lightCyan.wrap(release.version)}.
Please bump your version number and try again.

You can manage this release in the ${link(uri: uri, message: 'QuickPatch Console')}''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }
...
Please create a release using "quickpatch release" and try again.
''');
      throw ProcessExit(ExitCode.software.code);
    }
...
Looked in:
  ...
  - build/app/intermediates/stripped_native_libs/{flavor}Release/out/lib''',
    ...
    // (https://github.com/letssuhail/quickpatch/issues/3388).
...
QuickPatch looked for libapp.so under ${archsDir.path} but every requested
...
configuration excludes every architecture QuickPatch was asked to build. Either
relax those filters or pass `--target-platform=<archs>` to restrict QuickPatch
...
aar artifact already exists, continuing...''');
  ...
  /// Registers and uploads macOS release artifacts to the QuickPatch server.
  ...
  /// Publishes a patch to the QuickPatch server. This consists of creating a
  ...
  Never _handleErrorAndExit(
    ...
    if (error is CodePushUpgradeRequiredException) {
      progress?.fail();
      logger
        ..err('Your version of quickpatch is out of date.')
        ..info(
          '''Run ${lightCyan.wrap('quickpatch upgrade')} to get the latest version.''',
        );
