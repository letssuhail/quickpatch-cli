...
import 'package:quickpatch_cli/src/auth/ci_token.dart';
import 'package:quickpatch_cli/src/auth/endpoints/endpoints.dart';
import 'package:quickpatch_cli/src/auth/quickpatch_oauth.dart' as quickpatch_oauth;
...
/// The environment variable that holds the QuickPatch CI token.
const quickpatchTokenEnvVar = 'SHOREBIRD_TOKEN';
...
/// Callback for obtaining QuickPatch access credentials via loopback login.
...
class AuthenticatedClient extends http.BaseClient {
  ...
  Future<oauth2.AccessCredentials> _refreshForProvider(
    ...
    try {
      // QuickPatch uses its own refresh flow; Google and Microsoft use the
      // standard OAuth refresh. This branching can be removed once the
      // Google/Microsoft providers are fully removed from the CLI.
      if (authProvider == AuthProvider.quickpatch) {
        return await quickpatch_oauth.refreshQuickPatchCredentials(
          credentials,
          _baseClient,
      ...
      logger
        ...
        ..info(
          '''Try logging out with ${lightBlue.wrap('quickpatch logout')} and logging in again.''',
        )
        ..detail(e.toString())
        ..detail(s.toString());
...
class Auth {
  ...
  Auth({
    ...
    CodePushClientBuilder? buildCodePushClient,
       ...
       _authServiceUri = authServiceUri ?? quickpatchEnv.authServiceUri,
       _obtainCredentialsViaLoopbackLogin =
           obtainCredentialsViaLoopbackLogin ??
           quickpatch_oauth.obtainCredentialsViaLoopbackLogin,
  ...
  /// Logs out the user.
  ///
  /// If a QuickPatch refresh token is available, revokes the server-side
  ...
  void _loadCredentials() {
    // QuickPatch: prefer QUICKPATCH_TOKEN, fall back to legacy SHOREBIRD_TOKEN.
    final envToken =
        platform.environment['QUICKPATCH_TOKEN'] ??
        platform.environment[quickpatchTokenEnvVar];
    if (envToken != null) {
      final trimmed = envToken.trim();
      logger.detail('[env] $quickpatchTokenEnvVar detected');

      // New API key format — accept both qp_api_ (QuickPatch) and sb_api_ (legacy).
      if (trimmed.startsWith('qp_api_') || trimmed.startsWith('sb_api_')) {
        _apiKey = trimmed;
        logger.detail('[env] $quickpatchTokenEnvVar parsed as API key');
        return;
      }
      ...
      try {
        ...
        logger.warn(
          ...
          'release. '
          'Create an API key at https://console.quickpatch.dev instead.',
        );
      ...
      logger.detail('[env] $quickpatchTokenEnvVar parsed as legacy CiToken');
      return;
    }
...
extension OauthAuthProvider on Jwt {
  ...
  AuthProvider get authProvider {
    if (payload.iss == quickpatchEnv.jwtIssuer) {
      return AuthProvider.quickpatch;
...
extension OauthValues on AuthProvider {
  ...
  oauth2.AuthEndpoints get authEndpoints => switch (this) {
    ...
    (AuthProvider.quickpatch) => throw UnsupportedError(
      'QuickPatch auth does not use OAuth endpoints',
    ),
  };
    ...
    switch (this) {
      case AuthProvider.google:
        return oauth2.ClientId(
          /// QuickPatch CLI's OAuth 2.0 client ID for GCP.
          /// Replace with your own GCP OAuth client ID.
          const String.fromEnvironment(
            'QUICKPATCH_GOOGLE_CLIENT_ID',
            defaultValue: 'REPLACE_WITH_YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com',
          ),

          /// QuickPatch CLI's OAuth 2.0 client secret for GCP.
          /// For installed apps this is not truly secret — see:
          /// https://developers.google.com/identity/protocols/oauth2/native-app
          const String.fromEnvironment(
            'QUICKPATCH_GOOGLE_CLIENT_SECRET',
            defaultValue: 'REPLACE_WITH_YOUR_GOOGLE_CLIENT_SECRET',
          ),
        );
      case AuthProvider.microsoft:
        return oauth2.ClientId(
          /// QuickPatch CLI's OAuth 2.0 client ID for Azure/Entra.
          /// Replace with your own Azure app registration client ID.
          const String.fromEnvironment(
            'QUICKPATCH_MICROSOFT_CLIENT_ID',
            defaultValue: 'REPLACE_WITH_YOUR_AZURE_CLIENT_ID',
          ),
        );
      case AuthProvider.quickpatch:
        throw UnsupportedError('QuickPatch auth does not use a client ID');
    }
  }
  ...
  List<String> get scopes => switch (this) {
    ...
    // QuickPatch auth doesn't use scopes.
    (AuthProvider.quickpatch) => [],
  };
}
