...
const quickpatchTokenEnvVar = 'QUICKPATCH_TOKEN';
...
class Auth {
  ...
  void _loadCredentials() {
    final envToken = platform.environment[quickpatchTokenEnvVar];
    if (envToken != null) {
      final trimmed = envToken.trim();
      logger.detail('[env] $quickpatchTokenEnvVar detected');

      // API key format.
      if (trimmed.startsWith('qp_api_')) {
        _apiKey = trimmed;
      ...
      try {
        ...
        logger.warn(
          '$quickpatchTokenEnvVar contains a legacy CI token from '
          '`quickpatch login:ci`. '
