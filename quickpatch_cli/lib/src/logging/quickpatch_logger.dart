...
final loggerRef = create(QuickPatchLogger.new);

/// The [Logger] instance available in the current zone.
QuickPatchLogger get logger => read(loggerRef);

const _logFileName = 'quickpatch.log';

/// Where logs are written for the current QuickPatch CLI run. A new file will
/// be created for every run of the QuickPatch CLI, and will have the name
/// `timestamp_quickpatch.log`.
final File currentRunLogFile = (() {
  final timestamp = clock.now().millisecondsSinceEpoch;
  final file = File(
    p.join(quickpatchEnv.logsDirectory.path, '${timestamp}_$_logFileName'),
  );
...
/// {@endtemplate}
class QuickPatchLogger extends Logger {
  /// {@macro quickpatch_logger}
  QuickPatchLogger({super.level});
  ...
  void _failIfNonInteractive({String? promptText, String? hint}) {
    if (isInteractive && quickpatchEnv.canAcceptUserInput) return;
...
class _StaticProgress implements Progress {
  ...
  // No quickpatch code path calls cancel(); required by the Progress
