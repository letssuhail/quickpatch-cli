...
import 'package:quickpatch_cli/src/formatters/formatters.dart';
...
/// A command that creates a quickpatch patch for the provided target platforms.
/// `quickpatch patch --platforms=android,ios`
/// {@endtemplate}
class PatchCommand extends QuickPatchCommand {
  /// {@macro patch_command}
...
NOTE: this is ${styleBold.wrap('not')} recommended. Asset changes cannot be included in a patch can cause your app to behave unexpectedly.''';
  ...
  String get description =>
      'Creates a quickpatch patch for the provided target platforms.';
  ...
  Future<void> createPatch(Patcher patcher) async {
    ...
    } else if (quickpatchEnv.canAcceptUserInput) {
...
+-------------------------------------------------------------------------------+
''');
      lastBuiltFlutterRevision = quickpatchEnv.flutterRevision;
    ...
    try {
      await quickpatchFlutter.installRevision(revision: release.flutterRevision);
    } on Exception {
    ...
    if (obfuscationMapFile != null) {
      ...
      final shouldPreStripInGenSnapshot = await quickpatchFlutter
    ...
    if (hasObfuscate && !hasSplitDebugInfo) {
      extraBuildArgs.add(
        '--split-debug-info=${p.join('build', 'quickpatch', 'symbols')}',
      );
    }
...
''');
  ...
  void assertReleaseContainsPlatform({
    ...
    if (!contains) {
      ...
      logger.err(
        '''No release exists for $platformName in release version ${release.version}. Please run quickpatch release $platformName to create one.''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }
