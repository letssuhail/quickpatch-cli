...
class AndroidPatcher extends Patcher {
  ...
  /// https://github.com/letssuhail/updater/issues/211 was fixed in 3.24.2
...
See more info about the issue ${link(uri: Uri.parse('https://github.com/letssuhail/updater/issues/211'), message: 'on Github')}
''';
...
Looked in:
  ...
  Future<Map<Arch, PatchArtifactBundle>> createPatchArtifacts({
    ...
    // Direct users to https://github.com/letssuhail/quickpatch/issues/2532
...
Please refer to ${link(uri: Uri.parse('https://github.com/letssuhail/quickpatch/issues/2532'))} for potential workarounds.''',
        );
      }),
    );
  ...
  Future<String> extractReleaseVersionFromArtifact(File artifact) async {
    return quickpatchAndroidArtifacts.extractReleaseVersionFromAppBundle(
      artifact.path,
    );
  }
}
