...
import 'package:quickpatch_cli/src/extensions/iterable.dart';
...
abstract class Patcher {
  ...
  static String lowLinkPercentageWarning(double linkPercentage) {
    return '''
${lightCyan.wrap('quickpatch patch')} was only able to share ${linkPercentage.toStringAsFixed(1)}% of Dart code with the released app.
...
''';
  ...
  static File get debugInfoFile {
    return File(p.join(quickpatchEnv.buildDirectory.path, 'patch-debug.zip'));
  }
  ...
  ///
  /// Context: https://github.com/letssuhail/quickpatch/issues/2270
