...
To change the version of this release, change your app's version in your pubspec.yaml.''',
    ...
    if (generateApk && splitApk) {
      logger
        ..err(
          'QuickPatch does not support the split-per-abi option at this time',
        )
        ..info(
          '''
...
Please comment and upvote ${link(uri: Uri.parse('https://github.com/letssuhail/quickpatch/issues/1141'))} if you would like quickpatch to support this.''',
        );
  ...
  Future<String> getReleaseVersion({
    ...
    try {
      releaseVersion = await quickpatchAndroidArtifacts
  ...
  Future<void> uploadReleaseArtifacts({
    ...
    final aabFile = quickpatchAndroidArtifacts.findAab(
      project: projectRoot,
      flavor: flavor,
    );
  ...
  String get postReleaseInstructions {
    ...
    if (generateApk) {
      final apkFile = quickpatchAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
      apkText = generateApk
