...
/// {@endtemplate}
class ReleaseCommand extends QuickPatchCommand {
...
of the iOS app that is using this module. (aar and ios-framework only)''',
  ...
  String get description =>
      'Creates a quickpatch release for the provided target platforms.';
  ...
  /// The workflow to create a new release for a QuickPatch app.
  ///
  ...
  Future<void> createRelease(Releaser releaser) async {
    ...
    try {
      await quickpatchValidator.validateFlavors(
        flavorArg: flavor,
    ...
    try {
      await quickpatchFlutter.installRevision(revision: targetFlutterRevision);
    } on Exception {
    ...
    final releaseFlutterQuickPatchEnv = quickpatchEnv.copyWith(
    ...
    return await runScoped(
      () async {
        ...
        final flutterVersionString = await quickpatchFlutter
...
✅ Published Release ${release.version}!''')
      ...
      values: {quickpatchEnvRef.overrideWith(() => releaseFlutterQuickPatchEnv)},
    );
  }
  ...
  Future<void> assertArgsAreValid(Releaser releaser) async {
    ...
    if (quickpatchYaml?.patchVerification != null && !hasPublicKey) {
      logger.warn(
        'patch_verification is set in quickpatch.yaml but '
    ...
    final version = await quickpatchFlutter.resolveFlutterVersion(
      flutterVersionArg,
    );
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}''');
  ...
  /// [quickpatchEnv]. Will exit with [ExitCode.software] if the version
  /// specified by the user is not found/supported.
  Future<String> resolveTargetFlutterRevision() async {
    if (flutterVersionArg == 'latest') return quickpatchEnv.flutterRevision;

    // Fetch the latest remote refs so that release branch pointers
    // (e.g. flutter_release/3.38.5) are up to date.
    await quickpatchFlutter.fetchRemoteRefs();

    final String? revision;
    try {
      revision = await quickpatchFlutter.resolveFlutterRevision(
        flutterVersionArg,
      );
...
$error''');
    ...
    if (revision == null) {
      final openIssueLink = link(
        uri: Uri.parse(
          'https://github.com/letssuhail/quickpatch/issues/new?assignees=&labels=feature&projects=&template=feature_request.md&title=feat%3A+',
        ),
        message: 'open an issue',
      );
      logger.err('''
Version $flutterVersionArg not found. Please $openIssueLink to request a new version.
Use `quickpatch flutter versions list` to list available versions.
''');
      throw ProcessExit(ExitCode.software.code);
    }
  ...
  Future<void> ensureVersionIsReleasable({
    ...
    if (existingRelease != null) {
      ...
      if (existingRelease.flutterRevision != flutterRevision) {
        final flutterVersion = await quickpatchFlutter.getVersionForRevision(
          flutterRevision: flutterRevision,
        );

        final formattedCurrentReleaseVersion = quickpatchFlutter.formatVersion(
          revision: flutterRevision,
          version: flutterVersion,
        );

        final formattedExistingReleaseVersion = quickpatchFlutter.formatVersion(
...
To resolve this issue, you can:
  ...
  * Bump the release version and re-run the release command with the desired Flutter version.''',
  ...
  Future<void> printReleaseSummary({
    ...
    final flutterVersionString = await quickpatchFlutter.getVersionAndRevision();
...
''');
    ...
    if (confirm && quickpatchEnv.canAcceptUserInput) {
