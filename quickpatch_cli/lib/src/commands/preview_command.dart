...
/// `quickpatch preview` command.
/// {@endtemplate}
class PreviewCommand extends QuickPatchCommand {
...
This is only applicable when previewing Android releases.''',
  ...
  Future<int> run() async {
    ...
    final flavors = quickpatchYaml?.flavors;

    if (results.wasParsed('app-id')) {
      appId = results['app-id'] as String;
    } else if (quickpatchYaml != null && flavors == null) {
  ...
  Future<int> installAndLaunchMacos({
    ...
    // 2025-01-31 09:53:22.889 Df macos_sandbox[22535:1d268] [dev.quickpatch:updater::cache::patch_manager] [quickpatch] No public key provided, skipping signature verification
...
Skipping the ios-deploy fallback because it does not support iOS 17 or later.''',
  ...
  /// Sets the channel property in the quickpatch.yaml file inside the Windows
  /// app whose root directory is [appDirectory].
  Future<void> setChannelOnWindowsApp({
    required Directory appDirectory,
    required String channel,
  }) async {
    final quickpatchYamlFile = File(
      p.join(appDirectory.path, 'data', 'flutter_assets', 'quickpatch.yaml'),
    );

    await _maybeSetChannelInQuickPatchYaml(
      channel: channel,
      quickpatchYamlFile: quickpatchYamlFile,
    );
  }

  /// Sets the channel property in the quickpatch.yaml file inside the Runner.app
  Future<void> setChannelOnRunner({
    required Directory runnerDirectory,
    required String channel,
  }) async {
    await Isolate.run(() async {
      final quickpatchYaml = File(
        p.join(
      ...
      if (!quickpatchYaml.existsSync()) {
        throw Exception('Unable to find quickpatch.yaml');
      }

      await _maybeSetChannelInQuickPatchYaml(
        channel: channel,
        quickpatchYamlFile: quickpatchYaml,
      );
    });
  }

  /// Sets the channel property in the quickpatch.yaml file inside a macOS app.
  ...
  /// Unzips the `.aab` and sets the channel property in the quickpatch.yaml
  ...
  Future<void> setChannelOnAab({
    ...
    await Isolate.run(() async {
      ...
      final didUpdateQuickPatchYaml = await _maybeSetChannelInQuickPatchYaml(
        channel: channel,
        quickpatchYamlFile: quickpatchYamlFile,
      );

      if (!didUpdateQuickPatchYaml) {
        return;
      }
  ...
  /// Sets the channel property in the quickpatch.yaml file if none is set.
  Future<void> setChannelOnLinuxApp({
    required String channel,
    required Directory bundleDirectory,
  }) async {
    final quickpatchYamlFile = File(
      p.join(bundleDirectory.path, 'data', 'flutter_assets', 'quickpatch.yaml'),
    );
  ...
  /// Sets the channel property in the quickpatch.yaml file if none is set or
  /// different from the provided channel.
  static Future<bool> _maybeSetChannelInQuickPatchYaml({
    required String channel,
    required File quickpatchYamlFile,
  }) async {
    if (!quickpatchYamlFile.existsSync()) {
      throw Exception('Unable to find quickpatch.yaml');
    }

    final yamlText = quickpatchYamlFile.readAsStringSync();
    ...
    quickpatchYamlFile.writeAsStringSync(yamlEditor.toString(), flush: true);
    return true;
  }
