...
// `quickpatch_build_trace`'s `TraceSchema`; if you add a new bucket
...
import 'package:quickpatch_cli/src/artifact_builder/duration_distribution.dart';

/// Summary of a Chrome Trace Event Format build trace.
///
/// Aggregate [Duration] timings and small integer counters, suitable
/// for uploading to QuickPatch's servers as part of release telemetry.
class BuildTraceSummary {
  ...
  BuildTraceSummary({
    ...
    required this.quickpatchOverhead,
  ...
  factory BuildTraceSummary.fromEvents(
    ...
    return _buildSummary(
      acc,
      platform: platform,
      quickpatchOverhead: quickpatchOverhead,
  ...
  static BuildTraceSummary _buildSummary(
    ...
    return BuildTraceSummary(
      platform: platform,
      total: flutterBuild + (quickpatchOverhead ?? Duration.zero),
  ...
  /// QuickPatch overhead computed from it) should parse once and pass the
  ...
  /// Total command wall-clock (Flutter build + QuickPatch overhead).
  ...
  /// Wall-clock time the QuickPatch CLI spent around Flutter. Null when
  /// the caller couldn't compute it.
  final Duration? quickpatchOverhead;

  /// Network I/O time and request counts, summed across QuickPatch-side
  ...
  /// QuickPatch CLI's wall-clock time around Flutter with network I/O
  /// subtracted — i.e. what QuickPatch spent doing local work (file I/O,
  /// hashing, archive assembly, aot_tools link/gen_snapshot bookkeeping
  /// outside their own spans). Null when [quickpatchOverhead] is null.
  ///
  ...
  /// overhead).
  Duration? get quickpatchLocal {
    final overhead = quickpatchOverhead;
  ...
  Map<String, Object?> toJson() => <String, Object?>{
    ...
    'quickpatchOverheadMs': quickpatchOverhead?.inMilliseconds,
    'quickpatchLocalMs': quickpatchLocal?.inMilliseconds,
...
/// Network I/O totals. Combined across QuickPatch-side (auth, artifact
...
/// `quickpatch_trace_init.gradle` TaskExecutionListener.
class GradleStats {
