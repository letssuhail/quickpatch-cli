...
import 'package:quickpatch_jwt/quickpatch_jwt.dart' show Jwt, JwtPayload;
...
const quickpatchJwtIssuer = 'https://auth.quickpatch.dev';
...
void main() {
    ...
    test('creates instance with default constructor', () {
      final instance = runScoped(
        ...
        values: {
          ...
          quickpatchEnvRef.overrideWith(QuickPatchEnv.new),
        },
      );
      expect(
  ...
  group('OauthAuthProvider', () {
    ...
    setUp(() {
      ...
      when(() => quickpatchEnv.jwtIssuer).thenReturn(quickpatchJwtIssuer);
    });
    ...
    group('authProvider', () {
      ...
      group('when issuer is auth.quickpatch.dev', () {
        setUp(() {
          when(() => payload.iss).thenReturn(quickpatchJwtIssuer);
        });

        test('returns AuthProvider.quickpatch', () {
          runWithOverrides(() {
            expect(
              jwt.authProvider,
              equals(AuthProvider.quickpatch),
            );
          });
        });
      });
  ...
  group(Auth, () {
    ...
    const refreshToken = 'quickpatch-token';
    ...
    setUp(() {
      ...
      when(() => quickpatchEnv.jwtIssuer).thenReturn(quickpatchJwtIssuer);
      when(
        () => quickpatchEnv.authServiceUri,
      ).thenReturn(Uri.parse('https://auth.quickpatch.dev'));
      when(() => quickpatchEnv.hostedUri).thenReturn(null);
    ...
    group('AuthenticatedClient', () {
      ...
      group('token', () {
        ...
        group('when token is QuickPatch', () {
          test(
            'refreshes via QuickPatch and uses new token',
            () async {
              when(() => httpClient.send(any())).thenAnswer(
                (_) async => http.StreamedResponse(
                  const Stream.empty(),
              ...
              when(
                ...
                (_) async => http.Response(
                  jsonEncode({
                    'access_token': quickpatchIdToken,
              ...
              expect(
                ...
                equals('Bearer $quickpatchIdToken'),
              );
              verify(
                () => httpClient.post(
                  Uri.parse('https://auth.quickpatch.dev/token'),
                  headers: any(named: 'headers'),
                  body: any(named: 'body'),
                ),
              ).called(1);
          ...
          group('when QuickPatch refresh fails', () {
            late AuthenticatedClient client;
            setUp(() {
              when(
                () => httpClient.post(
      ...
      group('credentials', () {
        ...
        group('when expired credentials have QuickPatch issuer', () {
          test(
            'refreshes via QuickPatch and uses new token',
            () async {
              when(() => httpClient.send(any())).thenAnswer(
        ...
        group('when QuickPatch credential refresh fails', () {
          late AuthenticatedClient client;
          setUp(() {
            when(
              () => httpClient.post(
    ...
    group('client', () {
      ...
      group('when SHOREBIRD_TOKEN is an API key', () {
        ...
        test('parses as API key and sets isAuthenticated', () {
          ...
          verify(
            () => logger.detail(
              '[env] $quickpatchTokenEnvVar parsed as API key',
            ),
          ).called(1);
        });
      ...
      group('when token is invalid', () {
        setUp(() {
          when(() => platform.environment).thenReturn(<String, String>{
            quickpatchTokenEnvVar: 'not a base64 string',
          });
        });
        ...
        test(
          ...
          () async {
            ...
            verifyNever(
              () => logger.detail(
                '[env] $quickpatchTokenEnvVar parsed as legacy CiToken',
              ),
            );
          },
        );
...
''',
      ...
      test('returns an authenticated client '
        ...
        when(() => platform.environment).thenReturn(<String, String>{
          quickpatchTokenEnvVar: ciToken.toBase64(),
        });
        auth = buildAuth();
        final client = auth.client;
        expect(client, isA<http.Client>());
  ...
  group('OauthValues', () {
    group('clientId', () {
      ...
      test('throws UnsupportedError for quickpatch', () {
        expect(
          () => AuthProvider.quickpatch.clientId,
          throwsA(
            isA<UnsupportedError>().having(
              (e) => e.message,
              'message',
              'QuickPatch auth does not use a client ID',
            ),
          ),
        );
      });
    ...
    group('authEndpoints', () {
      test('throws UnsupportedError for quickpatch', () {
        expect(
          () => AuthProvider.quickpatch.authEndpoints,
