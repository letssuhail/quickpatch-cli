...
import 'package:quickpatch_cli/src/auth/quickpatch_oauth.dart';
...
String _buildTestJwt({String issuer = 'https://auth.quickpatch.dev'}) {
...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => quickpatchEnv.jwtIssuer,
    ).thenReturn('https://auth.quickpatch.dev');
  });
  ...
  group('obtainCredentialsViaLoopbackLogin', () {
    ...
    final authBaseUrl = Uri.parse('https://auth.quickpatch.dev');
    ...
    test('constructs correct login URL with continue parameter', () async {
      ...
      expect(loginUri.host, equals('auth.quickpatch.dev'));
    ...
    test('handles authBaseUrl with trailing slash', () async {
      final authBaseUrlWithSlash = Uri.parse('https://auth.quickpatch.dev/v1/');
      final testJwt = _buildTestJwt(issuer: 'https://auth.quickpatch.dev/v1/');
      when(
        () => quickpatchEnv.jwtIssuer,
      ).thenReturn('https://auth.quickpatch.dev/v1/');
  ...
  group('refreshQuickPatchCredentials', () {
    late MockHttpClient httpClient;
    final authBaseUrl = Uri.parse('https://auth.quickpatch.dev');
