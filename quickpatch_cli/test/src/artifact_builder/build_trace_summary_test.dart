...
void main() {
  group(BuildTraceSummary, () {
    test('empty events → zero summary', () {
      ...
      expect(s.quickpatchOverhead, isNull);
    ...
    test('android trace → nested gradle + android stats', () {
      ...
      final s = BuildTraceSummary.fromEvents(
        events,
        platform: 'android',
        quickpatchOverhead: const Duration(milliseconds: 500),
      );

      // Top level
      expect(s.flutterBuild, const Duration(milliseconds: 3005));
      expect(s.quickpatchOverhead, const Duration(milliseconds: 500));
      expect(s.total, const Duration(milliseconds: 3505));
      // quickpatchLocal = overhead 500 − network 300 = 200
      expect(s.quickpatchLocal, const Duration(milliseconds: 200));
