...
void main() {
  group(ArtifactBuilder, () {
    ...
    late QuickPatchProcessResult pubGetProcessResult;
    ...
    R runWithOverrides<R>(R Function() body) {
      return runScoped(
        ...
        values: {
          ...
          quickpatchTracerRef.overrideWith(QuickPatchTracer.new),
    ...
    setUpAll(() {
      registerFallbackValue(FakeQuickPatchProcess());
    ...
    setUp(() {
      ...
      // `--quickpatch-trace` so that trace tests pass when tracing is enabled.
      ...
      when(
        ...
        (_) async => QuickPatchProcessResult(
          ...
          stdout: '--quickpatch-trace',
          stderr: '',
        ),
      );
      when(
        () => quickpatchProcess.stream(
          any(),
          any(),
      ...
      when(() => quickpatchEnv.flutterRevision).thenReturn('1234');
      when(
...
'''),
      ...
      test(
        ...
        () async {
          ...
          await runWithOverrides(() async {
            ...
            final flowStarts = quickpatchTracer.events
                .where((e) => e['ph'] == 's')
                .toList();
        ...
        verify(
          () => quickpatchProcess.stream(
            ...
            [
              ...
              'appbundle',
              '--release',
              '--quickpatch-trace=$expectedTracePath',
            ],
            environment: any(named: 'environment'),
            runInShell: false,
            onStart: any(named: 'onStart'),
        ...
        setUp(() {
          when(
            () => quickpatchProcess.stream(
              ...
              environment: {'QUICKPATCH_PUBLIC_KEY': base64PublicKey, 'SHOREBIRD_PUBLIC_KEY': base64PublicKey},
              runInShell: false,
              onStart: any(named: 'onStart'),
            ),
          ).thenAnswer((_) async => ExitCode.success.code);
    ...
    group('buildApk', () {
      ...
      group('when Flutter supports build tracing', () {
        ...
        test(
          'passes --trace with a path under build/quickpatch/debug',
          () async {
            await runWithOverrides(() async {
              await builder.prepareBuildTrace(platform: 'android');
        ...
        test(
          ...
          () async {
            ...
            expect(summary['quickpatchOverheadMs'], isNonNegative);
    ...
    group('buildAar', () {
      ...
      test(
        'skips --quickpatch-trace when help probe shows no support',
        () async {
          when(
            () => quickpatchFlutter.resolveFlutterVersion(any()),
          ).thenAnswer((_) async => Version(3, 41, 7));
    ...
    group('buildLinuxApp', () {
      ...
      setUp(() {
        ...
        when(
          () => quickpatchProcess.stream('flutter', [
            'build',
            'linux',
            '--release',
          ], runInShell: any(named: 'runInShell')),
        ...
        test('provides public key as environment variable', () async {
          ...
          verify(
            () => quickpatchProcess.stream(
              'flutter',
              ['build', 'linux', '--release'],
              environment: {'QUICKPATCH_PUBLIC_KEY': publicKey, 'SHOREBIRD_PUBLIC_KEY': publicKey},
              runInShell: false,
              onStart: any(named: 'onStart'),
            ),
          ).called(1);
...
Reason: Exited with code 70.'''),
    ...
    group('prepareBuildTrace', () {
      test(
        ...
        () async {
          // quickpatchFlutter.resolveFlutterVersion default in setUp is 3.0.0,
      ...
      test(
        ...
        () async {
          ...
          when(() => quickpatchEnv.flutterRevision).thenReturn(
    ...
    group('writeBuildTraceSummary', () {
      test('logs detail and returns when trace file is missing', () async {
        ...
        await runWithOverrides(() async {
          ...
          // older Flutter that accepts --quickpatch-trace but silently
