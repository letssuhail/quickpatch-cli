...
void main() {
  group(QuickPatchTracer, () {
    late QuickPatchTracer tracer;

    setUp(() {
      tracer = QuickPatchTracer();
    });

    test('addNetworkEvent writes a cat=network span on network tid', () {
      tracer.addNetworkEvent(
        name: 'GET api.quickpatch.dev',
    ...
    group('mergeInto', () {
      ...
      setUp(() {
        tempDir = Directory.systemTemp.createTempSync('quickpatch_tracer_test_');
  ...
  group('quickpatchTracerRef', () {
    test('resolves to a QuickPatchTracer inside a scope', () {
      final tracer = runScoped(
        () => quickpatchTracer,
        values: {quickpatchTracerRef},
      );
      expect(tracer, isA<QuickPatchTracer>());
    });
  });
}
