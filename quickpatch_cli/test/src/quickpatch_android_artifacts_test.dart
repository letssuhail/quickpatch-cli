...
void main() {
  group(QuickPatchAndroidArtifacts, () {
    ...
    group('when no build folder exists', () {
      ...
      test('throws ArtifactNotFoundException for apks', () {
        final buildDir = Directory(
          p.join(projectRoot.path, 'build', 'app', 'outputs', 'flutter-apk'),
        );
        expect(
    ...
    group('when using no flavors', () {
      test('finds the app bundle flavors', () {
        ...
        final artifact = File(p.join(projectRoot.path, artifactPath))
          ..createSync(recursive: true);

        expect(
          quickpatchAndroidArtifacts
              .findAab(project: projectRoot, flavor: null)
              .path,
      ...
      test('finds the apk', () {
        ...
        expect(
          quickpatchAndroidArtifacts
              .findApk(project: projectRoot, flavor: null)
              .path,
          equals(artifact.path),
        );
      });
    ...
    group('when using single-dimensional flavor', () {
      test('finds the app bundle', () {
        ...
        expect(
          quickpatchAndroidArtifacts
              .findAab(project: projectRoot, flavor: flavor)
              .path,
          equals(artifact.path),
        );
      });
      ...
      test('finds the apk', () {
        ...
        expect(
          quickpatchAndroidArtifacts
              .findApk(project: projectRoot, flavor: flavor)
              .path,
          equals(artifact.path),
        );
      });
    ...
    group(
      'when using multi-dimensional flavors and multi-word flavor name',
      () {
        test('finds the app bundle', () {
          final artifactPath = p.join(
            'build',
        ...
        expect(
          ...
          throwsA(
            isA<MultipleArtifactsFoundException>().having(
              ...
              'message',
              equals(
                'Multiple artifacts found in ${buildDir.path}: '
    ...
    group('aarLibraryPath', () {
      test('returns path to AAR library', () {
        expect(
          runWithOverrides(() => QuickPatchAndroidArtifacts.aarLibraryPath),
          equals(p.join(projectRoot.path, 'build', 'host', 'outputs', 'repo')),
        );
      });
    });
    ...
    group('aarArtifactDirectory', () {
      test('returns path to AAR artifact directory', () {
        expect(
          runWithOverrides(
            () => QuickPatchAndroidArtifacts.aarArtifactDirectory(
              buildNumber: '1',
    ...
    group('aarArtifactPath', () {
      test('returns path to AAR artifact', () {
        final result = runWithOverrides(
          () => QuickPatchAndroidArtifacts.aarArtifactPath(
            packageName: 'com.example',
            buildNumber: '1',
          ),
        );
