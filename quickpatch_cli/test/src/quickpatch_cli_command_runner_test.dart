...
import 'package:quickpatch_cli/src/logging/logging.dart' hide logger;
...
void main() {
  group(QuickPatchCliCommandRunner, () {
    ...
    late QuickPatchCliCommandRunner commandRunner;
    ...
    R runWithOverrides<R>(R Function() body) {
      return runScoped(
        ...
        values: {
          ...
          quickpatchVersionRef.overrideWith(() => quickpatchVersion),
        },
      );
    }
    ...
    setUp(() {
      ...
      when(quickpatchVersion.isTrackingStable).thenAnswer((_) async => true);
      commandRunner = runWithOverrides(QuickPatchCliCommandRunner.new);
    });
    ...
    test('handles UsageException', () async {
      ...
      verify(
        () => logger.info(
          any(that: contains('Usage: quickpatch <command> [arguments]')),
        ),
      ).called(1);
    });
...
${lightCyan.wrap('quickpatch release android -- --no-pub lib/main.dart')}'''),
      ).called(1);
...
Engine • revision $quickpatchEngineRevision'''),
        ...
        // https://github.com/letssuhail/quickpatch/issues/2260
    ...
    group('update check', () {
      group('when running upgrade command', () {
        ...
        test('does not check for update', () async {
          ...
          expect(result, equals(ExitCode.success.code));
          verifyNever(() => quickpatchVersion.isTrackingStable());
          verifyNever(() => quickpatchVersion.isLatest());
        });
      });
      ...
      group('when tracking the stable branch', () {
        ...
        test(
          ...
          () async {
            when(quickpatchVersion.isLatest).thenThrow(Exception('error'));
            final result = await runWithOverrides(
              () => commandRunner.run(['--version']),
            );
            expect(result, equals(ExitCode.success.code));
        ...
        group('when update is available', () {
          test('logs update message', () async {
            ...
            verify(
              () => logger.info('A new version of quickpatch is available!'),
            ).called(1);
            verify(
              () => logger.info(
                'Run ${lightCyan.wrap('quickpatch upgrade')} to upgrade.',
              ),
            ).called(1);
      ...
      group('when not tracking the stable branch', () {
        setUp(() {
          when(
            quickpatchVersion.isTrackingStable,
    ...
    group('--json', () {
      ...
      group('on parse-time errors', () {
        test('emits JSON error envelope on unknown command', () async {
          ...
          final error = json['error'] as Map<String, dynamic>;
          expect(error['code'], equals('usage_error'));
          expect(error['hint'], equals('Run: quickpatch --help'));
          final meta = json['meta'] as Map<String, dynamic>;
          expect(meta['command'], equals('quickpatch'));
...
class _TestCommand extends QuickPatchCommand {
...
class _ThrowingCommand extends QuickPatchCommand {
  @override
...
class _PromptRequiredCommand extends QuickPatchCommand {
...
/// [ansiOutputEnabled]).
class _CaptureModeCommand extends QuickPatchCommand {
