...
void main() {
  group(PreviewCommand, () {
    ...
    Future<void> setupAndroidQuickPatchYaml(Invocation invocation) async {
      File(
          p.join(
    ...
    Future<File> quickpatchYamlFileFromAab(File aab) async {
      final tempDir = Directory.systemTemp.createTempSync();
      final aabDirectory = Directory(p.join(tempDir.path, 'app-release'))
        ..createSync(recursive: true);
    ...
    setUp(() {
      ...
      when(() => quickpatchEnv.getQuickPatchYaml()).thenReturn(null);
      when(
      ...
      group(
        ...
        () {
          setUp(() {
            ...
            ).thenAnswer(setupAndroidQuickPatchYaml);
      ...
      group('setChannelOnAab', () {
        ...
        group('when channel is not set', () {
          group('when target channel is  production', () {
            test('does not change quickpatch.yaml', () async {
              ...
              final updatedQuickPatchYamlFile = await quickpatchYamlFileFromAab(
                aabFile,
              );
              expect(
                updatedQuickPatchYamlFile.readAsStringSync(),
                'app_id: $appId\n',
              );
            });
          });
          ...
          group('when target channel is not production', () {
            test('sets quickpatch.yaml channel to target channel', () async {
              aabFile = await createAabFile(channel: null);
              await runWithOverrides(
                () =>
              ...
              expect(updatedQuickPatchYamlFile.readAsStringSync(), '''
app_id: $appId
channel: live
''');
            });
...
''');
      ...
      group('when unable to find quickpatch.yaml', () {
        test('exits with code 70', () async {
          final result = await runWithOverrides(command.run);
          expect(result, equals(ExitCode.software.code));
          verify(
            () => progress.fail('Exception: Unable to find quickpatch.yaml'),
          ).called(1);
        });
      });
      ...
      group('when in a quickpatch project without flavors', () {
        setUp(() {
          when(
            () => quickpatchEnv.getQuickPatchYaml(),
          ).thenReturn(const QuickPatchYaml(appId: 'test-app-id'));
      ...
      group('when in quickpatch project with flavors', () {
        setUp(() {
          when(() => quickpatchEnv.getQuickPatchYaml()).thenReturn(
    ...
    group('ios', () {
      ...
      group('when install/launch succeeds (production)', () {
        late File quickpatchYaml;
        setUp(() {
          quickpatchYaml = setupIOSQuickPatchYaml();
          when(
            () => iosDeploy.installAndLaunchApp(
              bundlePath: any(named: 'bundlePath'),
              deviceId: any(named: 'deviceId'),
        ...
        test('exits with code 0', () async {
          ...
          expect(quickpatchYaml.readAsStringSync(), equals('app_id: $appId'));
        });
      });
      ...
      group('when install/launch succeeds (staging)', () {
        ...
        test('exits with code 0', () async {
          ...
          expect(
            quickpatchYaml.readAsStringSync(),
            equals('''
app_id: $appId
...
'''),
    ...
    group('linux', () {
      ...
      setUp(() {
        ...
        when(
          () => quickpatchProcess.start(any(), any()),
        ).thenAnswer((_) async => process);
        when(() => quickpatchProcess.run(any(), any())).thenAnswer(
      ...
      group('when preview artifact is not cached', () {
        ...
        test('downloads and launches artifact', () async {
          ...
          verify(
            () =>
                quickpatchProcess.start(any(that: endsWith(executableName)), []),
          ).called(1);
      ...
      group('when preview artifact is cached', () {
        ...
        test('launches cached artifact', () async {
          ...
          verify(() => quickpatchProcess.run('chmod', any())).called(1);
          verify(
            () =>
                quickpatchProcess.start(any(that: endsWith(executableName)), []),
          ).called(1);
    ...
    group('macos', () {
      ...
      group('staging', () {
        ...
        setUp(() {
          ...
          quickpatchYaml = setupMacosQuickPatchYaml();
        });

        test('sets channel in quickpatch.yaml', () async {
          final result = await runWithOverrides(command.run);
          expect(result, equals(ExitCode.success.code));
...
'''),
      ...
      group('when process completes with exit code 0', () {
        setUp(setupMacosQuickPatchYaml);
...
2025-01-31 10:15:29.808 Df macos_sandbox[33557:2cc12] [dev.quickpatch:updater::updater] [quickpatch] Reporting successful launch.
'''
                .split('\n');

        setUp(() {
          setupMacosQuickPatchYaml();
        ...
        test('removes noisy lines from log output', () async {
          ...
          verify(
            () => logger.info(
              '''[dev.quickpatch:updater::updater] [quickpatch] Reporting successful launch.''',
            ),
          ).called(1);
          verifyNever(
            () => logger.info(
      ...
      group('non-UTF8 handling', () {
        ...
        test('handles non-UTF8 bytes in log stream without throwing', () async {
          ...
          final invalidUtf8Bytes = [
            ...utf8.encode(
              '2025-01-31 09:53:22.889 Df app[22535:1d268] [quickpatch] Hello',
            ),
            0xFF,
            0x0A, // newline
          ];
          ...
          verify(() => logger.info('[quickpatch] Hello\uFFFD')).called(1);
        });
      });
    });
    ...
    group('windows', () {
      ...
      void createQuickPatchYaml({String? channel}) {
        final quickpatchYaml =
            File(
                p.join(
              ...
              ..writeAsStringSync('app_id: $appId', flush: true);
        if (channel != null) {
          quickpatchYaml.writeAsStringSync(
            'channel: $channel',
            flush: true,
      ...
      group('when preview artifact is not cached', () {
        ...
        test('downloads and launches artifact', () async {
          ...
          verify(
            () => quickpatchProcess.start(any(that: endsWith('runner.exe')), []),
          ).called(1);
          verify(() => logger.info('hello world')).called(1);
          verify(() => logger.err('hello error')).called(1);
        });
