...
import 'package:quickpatch_cli/src/archive_analysis/apple_archive_differ.dart';
...
void main() {
  group(IosPatcher, () {
    ...
    setUp(() {
      ...
      when(() => quickpatchEnv.iosSupplementDirectory).thenReturn(
        Directory(p.join(projectRoot.path, 'build', 'quickpatch', 'ios')),
      );
      when(() => quickpatchEnv.iosPodfileLockHash).thenReturn(null);
            ...
            group('when native diffs are not allowed', () {
              ...
              group(
                ...
                () {
                  setUp(() {
                    when(
                      () => quickpatchEnv.canAcceptUserInput,
                    ).thenReturn(false);
                  });
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}'''),
        ...
        group('when extraBuildArgs has obfuscation flags', () {
          ...
          test('includes obfuscation flags in build args', () async {
            ...
            patcher.extraBuildArgs = [
              ...
              '--split-debug-info=build/quickpatch/symbols',
            ];
            await runWithOverrides(patcher.buildPatchArtifact);
    ...
    group('createPatchArtifacts', () {
      ...
      void setUpProjectRootArtifacts() {
        ...
        File(
          p.join(projectRoot.path, 'build', 'ios', 'quickpatch', 'App.ct.link'),
        ).createSync(recursive: true);
        File(
          p.join(
            projectRoot.path,
      ...
      group('when uses linker', () {
        ...
        setUp(() {
          final quickpatchRoot = Directory.systemTemp.createTempSync();
          flutterDirectory = Directory(
            p.join(quickpatchRoot.path, 'bin', 'cache', 'flutter'),
          );
