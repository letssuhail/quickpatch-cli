...
environment:
  sdk: ">=2.19.0 <3.0.0"''';
    ...
    setUp(() {
      ...
      when(
        () => quickpatchEnv.getQuickPatchYamlFile(cwd: any(named: 'cwd')),
      ).thenReturn(quickpatchYamlFile);
      when(
        () => quickpatchEnv.getPubspecYamlFile(cwd: any(named: 'cwd')),
      ).thenReturn(pubspecYamlFile);
      when(() => quickpatchEnv.getFlutterProjectRoot()).thenReturn(projectRoot);
      when(
...
'''),
    ...
    test('throws software error when pubspec.yaml is malformed.', () async {
      ...
      when(() => quickpatchEnv.hasPubspecYaml).thenThrow(exception);
      final exitCode = await runWithOverrides(command.run);
      verify(
    ...
    test('throws software error when quickpatch.yaml already exists', () async {
      when(() => quickpatchEnv.hasQuickPatchYaml).thenReturn(true);
      final exitCode = await runWithOverrides(command.run);
      verify(
        () => logger.err(
          'A "quickpatch.yaml" file already exists and seems up-to-date.',
        ),
      ).called(1);
      verify(
        () => logger.info(
    ...
    test(
      ...
      () async {
        ...
        when(() => quickpatchYaml.flavors).thenReturn(null);
    ...
    test(
      ...
      () async {
        ...
        when(() => quickpatchEnv.getQuickPatchYaml()).thenReturn(quickpatchYaml);
        when(() => quickpatchYaml.appId).thenReturn(appId);
        when(() => quickpatchYaml.flavors).thenReturn({'dev': 'id-1'});
        when(
    ...
    test('does not prompt for name when unable to accept user input', () async {
      when(() => quickpatchEnv.canAcceptUserInput).thenReturn(false);
    ...
    test('--force overwrites existing quickpatch.yaml', () async {
      when(() => quickpatchEnv.hasQuickPatchYaml).thenReturn(true);
      when(() => argResults['force']).thenReturn(true);
      final exitCode = await runWithOverrides(command.run);
      verifyNever(
      ...
      verify(
        () => quickpatchYamlFile.writeAsStringSync(
    ...
    group('on non MacOS', () {
      ...
      test('creates quickpatch for an android-only app', () async {
        final exitCode = await runWithOverrides(command.run);
        expect(exitCode, equals(ExitCode.success.code));
        verify(
          () => quickpatchYamlFile.writeAsStringSync(
      ...
      test('creates quickpatch for an app without flavors', () async {
        when(
          () => gradlew.productFlavors(any()),
      ...
      test('creates quickpatch for an app with flavors', () async {
...
flavors:
  ...
  stable: ${appIds[1]}'''),
    ...
    test('creates quickpatch.yaml for an app without flavors', () async {
      await runWithOverrides(command.run);
      verify(
        () => quickpatchYamlFile.writeAsStringSync(
          any(that: contains('app_id: $appId')),
    ...
    group('creates quickpatch.yaml for an app with flavors', () {
  ...
  prod: test-appId-prod'''),
      ...
      group('with new flavors added', () {
        ...
        setUp(() {
          ...
          when(() => quickpatchEnv.getQuickPatchYaml()).thenReturn(quickpatchYaml);
          when(() => quickpatchYaml.appId).thenReturn(appId);
          when(() => quickpatchYaml.flavors).thenReturn(existingFlavors);
        });
        ...
        test('creates new flavor entries in quickpatch.yaml', () async {
...
flavors:
  ...
  d: test-appId-4'''),
    ...
    test('detects existing quickpatch.yaml in pubspec.yaml assets', () async {
...
''');
    ...
    test('ensures that addQuickPatchYamlToPubspecAssets is called', () async {
      when(() => quickpatchEnv.pubspecContainsQuickPatchYaml).thenReturn(false);
      await runWithOverrides(command.run);
